...
func getSubStore(cfg *config.Config, subPaths map[string]config.StorageConfig,
	...
	for route, storageConfig := range subPaths {
		...
		if storageConfig.StorageDriver == nil {
			...
			/* storageConfig.RootDirectory (the subpath's RootDirectory) is used for
			caching blobs locally. The upstream storage driver handles rootdirectory
			prefixing internally, so rootDir for zot's ImageStore must always be "/". */
			rootDir := "/"
